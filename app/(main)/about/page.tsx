  ...
  return <>
    ...
    <div className="about-editorial">
      ...
      <section className="about-records" aria-label="Experience and education">
        <div><h2>Experience</h2>{experience.map((item) => <article key={item.company}><p className="record-date">{item.period} / {item.location}</p><h3>{item.company}</h3><p>{item.role}</p></article>)}</div>
      ...
      <section className="about-brands" aria-label="Brands I've worked with">{brands.filter((brand) => brand.logo).map((brand) =>
        // eslint-disable-next-line @next/next/no-img-element
      ...
      <section className="about-approach"><blockquote>{philosophy.quote}</blockquote><p dangerouslySetInnerHTML={{ __html: sanitizeInlineHtml(philosophy.mission) }} /></section>
